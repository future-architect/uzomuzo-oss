...
// BuildScopeMap walks all components recursively and builds a mapping from
// normalized PURL to CycloneDX scope value ("required", "optional", "excluded").
// Components without a PURL or scope are skipped. Returns nil when no scope
// information is present (e.g., SBOMs generated by Trivy or syft).
func BuildScopeMap(components []Component) map[string]string {
	m := make(map[string]string)
	buildScopeMapRecursive(components, m, 0)
	if len(m) == 0 {
		return nil
	}
	return m
...
func buildScopeMapRecursive(components []Component, m map[string]string, depth int) {
	...
	for _, c := range components {
		if c.PURL != "" && c.Scope != "" {
			normalized := NormalizePURL(c.PURL)
			if normalized != "" {
				m[normalized] = c.Scope
			}
		}
		if len(c.Components) > 0 {
			buildScopeMapRecursive(c.Components, m, depth+1)
		}
	}
}
