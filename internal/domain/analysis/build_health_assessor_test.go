...
func TestBuildHealthAssessorService_Assess(t *testing.T) {
	tests := []struct {
		...
		{
			name: "below_min_two_signals_ungraded",
			scores: map[string]*ScoreEntity{
				"Branch-Protection": NewScoreEntity("Branch-Protection", 10, 10, ""),
				"Code-Review":       NewScoreEntity("Code-Review", 10, 10, ""),
			},
			wantLabel: string(BuildLabelUngraded),
			wantScore: "-1", // 2 evaluated < minEvaluatedSignals (3)
		},
		{
		...
		{
			name: "below_min_with_inconclusive",
			scores: map[string]*ScoreEntity{
...
func TestBuildHealthAssessorService_SignalCount(t *testing.T) {
	...
	// Should have 6 signals total (3 used + 3 absent)
	if len(res.Signals) != 6 {
		t.Errorf("signal count = %d, want 6", len(res.Signals))
	}
	usedCount := 0
