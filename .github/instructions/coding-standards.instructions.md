...
- **Deterministic Output from Map Iteration**: When iterating over a Go map to build ordered output (slices, rendered text), sort the map keys first. Go map iteration order is non-deterministic, leading to unstable CLI output between runs for the same input.
