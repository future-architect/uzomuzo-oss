...
- **Error Hints Must Be Ecosystem-Agnostic in Multi-Language Tools**: When a CLI or library supports multiple languages/ecosystems, error messages and hints must not reference a single ecosystem's file (e.g., `go.mod`) in a context that applies to all supported languages. Use generic phrasing or dynamically select the hint based on the detected ecosystem.
