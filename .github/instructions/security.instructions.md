...
- **Normalize Hostnames in SSRF Denylists and Cache Keys**: When checking hostnames against a denylist (loopback, metadata endpoints) or using them as cache/map keys, strip trailing dots (FQDN form) and lowercase the host before comparison. `localhost.` and `metadata.google.internal.` are equivalent to their non-dot forms but bypass naive string switches; mixed-case hosts like `GOPKG.IN` produce distinct cache keys from `gopkg.in`, causing redundant fetches and unbounded cache growth. Apply `strings.TrimRight(host, ".")` + `strings.ToLower` before denylist checks and cache-key construction.
