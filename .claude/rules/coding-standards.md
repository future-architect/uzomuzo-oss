...
- **Scope Dedup Exclusion Sets Per Entity**: When deduplicating items across multiple collections (e.g., advisories across StableVersion and MaxSemverVersion), build the exclusion set per collection — not by combining all collections into one. A combined set can incorrectly exclude items from one collection because they happen to exist in another, violating per-entity semantics.
- **Cache Results for Shared Inputs in Batch Processing**: When a batch loop calls an expensive function (API call, I/O) with the same logical input multiple times (same pointer, same resolved key), cache the result on first call and reuse it. This applies to both negative caching (404s for immutable lookups) and identity-based caching (multiple PURLs sharing the same dependency graph).
- **Use Bounded Worker Pools for Parallel I/O**: When parallelizing I/O across a variable number of targets, use a fixed-size worker pool (N goroutines consuming from a jobs channel) rather than launching one goroutine per target with a semaphore. The goroutine-per-target pattern creates O(N) blocked goroutines for large N. Workers must also check `ctx.Done()` to avoid blocking on a cancelled context.
- **Guard Nil Structs Before Method Calls**: When calling methods on struct pointers that may be nil (e.g., `ReleaseInfo.LatestVersionDetail()`), add a nil guard before the call. If one code path (e.g., JSON renderer) has the guard but another (e.g., CSV renderer) does not, the unguarded path will panic on nil input.
- **Use `utf8.RuneCountInString` for Display Width**: When computing terminal display widths or column padding, use `utf8.RuneCountInString` (not `len`) to count characters. `len` returns byte count, which is incorrect for multi-byte characters (box-drawing glyphs, emoji, CJK) and produces misaligned output.
- **Use Case-Insensitive Comparison for URL Components**: When detecting URL hosts or schemes, use `strings.EqualFold` or normalize to lowercase before comparison. URLs are case-insensitive per RFC 3986, and user/API data may use mixed case.
- **Clamp Computed Sizes to Zero**: When computing padding, margins, or other sizes by subtraction (e.g., `budget - used`), clamp the result to zero with `max(0, result)` rather than forcing a minimum positive value. A forced minimum makes output wider than the declared width when content already exceeds the budget.
- **Remove Unused Parameters from Signatures**: When a function parameter is no longer referenced in the body (due to refactoring), remove it from the signature immediately. Dead parameters add noise, confuse callers about required context, and accumulate as API debt.
- **Render All Populated Data Fields**: When building output (CLI, JSON, CSV), render every populated field in the data struct. Silently dropping fields (e.g., advisory URLs, dependency names) that are available prevents users from accessing useful information without any visible indication that data was omitted.
- **Consistent Data-Source Fallback Across Formats**: Output formats (table, JSON, CSV) must use the same data-source fallback chain as domain logic (e.g., stable → MaxSemver → PreRelease → Requested). Inconsistent fallback chains cause different formats to show different data for the same input.
