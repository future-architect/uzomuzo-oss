...
- **Use Bounded Worker Pools for Batch Concurrency**: When fetching data for multiple keys concurrently, use a fixed-size worker pool (N goroutines reading from a jobs channel) rather than spawning one goroutine per key with a semaphore. The semaphore pattern caps in-flight requests but still creates O(keys) goroutines, increasing memory and scheduler overhead for large inputs. Match the existing pattern in `depsdev/depsdev.go`.
- **Clone Shared References Before Assigning to Multiple Owners**: When batch-fetched results (maps, slices) are assigned to multiple domain objects that share the same lookup key, clone the data structure per assignment. Direct assignment shares the underlying instance, so a mutation in one owner silently corrupts all others.
