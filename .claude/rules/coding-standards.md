...
- **Comment-Code Consistency**: When changing implementation behavior (e.g., switching from three-dot to two-dot diff), update all comments and documentation that reference the old behavior in the same commit. This includes header comments in configuration files (YAML, JSON) — if a header describes a process or threshold, it must match the actual implementation.
